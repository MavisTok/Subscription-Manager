...
| 失败→恢复 | 是 | 通知已恢复正常 |
| 持续失败 | 否 | 不重复推送相同故障 |
| 冷却期内状态振荡 | 否 | 防止时好时坏频繁推送 |
| 24h 内超上限（熔断） | 否 | 彻底停止通知，窗口期过后自动恢复 |

- **冷却期** = max(任务间隔 × 3, 30分钟)，冷却期内状态振荡不会触发通知
- **熔断机制**：24h 滑动窗口内通知次数上限 = clamp(24h内最大执行次数 × 2, 5, 20)，达到上限后发送一条熔断告警并停止该任务后续通知，窗口期（24h）过后自动恢复

### 5. 定时调度

安装时自动按平台选择最优调度方式：

- **并发锁**：防止多个 cron_check 实例同时运行
- **独立时间戳兜底**：即使 `tasks.json` 的 `last_run` 写入失败，也不会重复触发任务
